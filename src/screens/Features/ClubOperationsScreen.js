...
export default function ClubOperationsScreen({ navigation, route }) {
  ...
  const [checklistTeamFilter, setChecklistTeamFilter] = useState("All");
  ...
  const filteredChecklists = useMemo(() => {
    return (checklists || []).filter((c) => {
      if (checklistTeamFilter === "All") return true;
      return c.teamId === checklistTeamFilter;
    });
  }, [checklists, checklistTeamFilter]);
    ...
    <>
      ...
      >
        <TouchableOpacity
          style={[
            styles.filterChip,
            checklistTeamFilter === "All" ? styles.filterChipActive : null,
          ]}
          onPress={() => setChecklistTeamFilter("All")}
        >
          <Text
            variant="small"
            weight="700"
            color={
              checklistTeamFilter === "All"
                ? theme.colors.white
                : theme.colors.text
            }
          >
        ...
        {(teamRows || []).map((team) => (
          <TouchableOpacity
            ...
            style={[
              styles.filterChip,
              checklistTeamFilter === team.id ? styles.filterChipActive : null,
            ]}
            onPress={() => setChecklistTeamFilter(team.id)}
          >
            <Text
              variant="small"
              weight="700"
              color={
                checklistTeamFilter === team.id
                  ? theme.colors.white
                  : theme.colors.text
              }
            >
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {tabIndex === 3 ? renderCreatePlanTab() : null}
          {tabIndex === 4 ? renderTrainingPlansTab() : null}
      </ScrollView>
